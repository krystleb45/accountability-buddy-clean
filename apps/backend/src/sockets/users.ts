...
if (!isRedisDisabled) {
  try {
    ...
    logger.error(
      `Failed to load Redis client for UsersSocket: ${(error as Error).message}`,
    )
  }
} else {
...
function usersSocket(io: Server, socket: Socket): void {
  ...
  if (!userId) {
    ...
    socket.emit("error", {
      msg: "Authentication error: User ID is missing or invalid.",
    })
    return
  }
  ...
  socket.on("userConnected", async (): Promise<void> => {
    try {
      ...
      logger.error(
        `Error in userConnected event for user ${userId}: ${(error as Error).message}`,
      )
  ...
  socket.on("disconnect", async (): Promise<void> => {
    try {
      ...
      logger.error(
        `Error in disconnect event for user ${userId}: ${(error as Error).message}`,
      )
    }
  })
  ...
  socket.on("updateStatus", async (newStatus: string): Promise<void> => {
    try {
      ...
      if (!validStatuses.includes(newStatus)) {
        logger.warn(
          `Invalid status update attempt for user ${userId}: ${newStatus}`,
        )
      ...
      logger.error(
        `Error in updateStatus event for user ${userId}: ${(error as Error).message}`,
      )
  ...
  socket.on("fetchOnlineUsers", async (): Promise<void> => {
    try {
      ...
      logger.info(
        `Online users sent to user ${userId} (${onlineUsers.length} users)`,
      )
    } catch (error) {
      logger.error(
        `Error fetching online users for user ${userId}: ${(error as Error).message}`,
      )
    ...
    async ({
      ...
      try {
        ...
        if (recipientSockets.length === 0) {
          logger.warn(
            `Private message failed: User ${recipientId} is not online`,
          )
        ...
        logger.info(
          `Private message from user ${userId} to user ${recipientId}`,
        )
      } catch (error) {
        logger.error(
          `Error in privateMessage event for user ${userId}: ${(error as Error).message}`,
        )
