...
export function chatSocketHandler(io: Server, socket: Socket): void {
  ...
  socket.on(
    "newMessage",
    (data: { chatId: string; message: string; userId: string }) => {
      const { chatId, message, userId } = data
      logger.info(
        `New message in chat ${chatId} from user ${userId}: ${message}`,
      )
  ...
  socket.on("joinChat", async (chatId: string) => {
    try {
      ...
      logger.error(
        `Error joining chat ${chatId} for user ${socket.id}: ${(error as Error).message}`,
      )
    }
  })
  ...
  socket.on("leaveChat", async (chatId: string) => {
    try {
      ...
      logger.error(
        `Error leaving chat ${chatId} for user ${socket.id}: ${(error as Error).message}`,
      )
    }
  })
