...
export function pollSocketHandler(io: Server, socket: Socket): void {
  ...
  socket.on(
    "votePoll",
    async (pollId: string, optionId: string, userId: string) => {
      try {
        ...
        if (error instanceof Error) {
          logger.error(
            `Error processing vote for poll ${pollId}: ${error.message}`,
          )
  ...
  socket.on("getPollResults", async (pollId: string) => {
    try {
      ...
      if (error instanceof Error) {
        logger.error(
          `Error fetching results for poll ${pollId}: ${error.message}`,
        )
