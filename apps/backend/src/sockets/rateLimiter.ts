...
if (!isRedisDisabled) {
  import("../config/redisClient")
    ...
    .catch((error) => {
      logger.error(
        `Failed to load Redis client for SocketRateLimiter: ${(error as Error).message}`,
      )
    })
} else {
  ...
  return async (socket: Socket, next: (err?: Error) => void): Promise<void> => {
    ...
    try {
      ...
      if (requests > maxRequests) {
        logger.warn(
          `Rate limit exceeded for user ${userId} on event ${eventName} (${requests}/${maxRequests})`,
        )
        return next(
          new Error(
            `Rate limit exceeded for ${eventName}. Please wait before trying again.`,
          ),
        )
      }

      logger.debug(
        `Rate limit check passed for user ${userId} on event ${eventName} (${requests}/${maxRequests})`,
      )
