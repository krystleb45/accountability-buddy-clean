...
function typingIndicatorSocket(_io: Server, socket: Socket): void {
  ...
  socket.on("startTyping", (typingData: TypingData): void => {
    try {
      ...
      logger.error(
        `Error processing 'startTyping': ${(error as Error).message}`,
      )
  ...
  socket.on("disconnect", (): void => {
    try {
      logger.info(
        `User ${userId} disconnected from typing indicator management`,
      )
      ...
      logger.error(
        `Error during user disconnection: ${(error as Error).message}`,
      )
    }
  })
}
