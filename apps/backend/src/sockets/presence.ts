...
if (!isRedisDisabled) {
  ...
  import("../config/redisClient")
    ...
    .catch((error) => {
      logger.error(
        `Failed to load Redis client for PresenceSocket: ${(error as Error).message}`,
      )
    })
} else {
...
function presenceSocket(_io: Server, socket: Socket): void {
  ...
  const markUserOnline = async (): Promise<void> => {
    try {
      ...
      logger.error(
        `Error marking user ${userId} as online: ${(error as Error).message}`,
      )
    }
  }
  ...
  const markUserOffline = async (): Promise<void> => {
    try {
      ...
      logger.error(
        `Error marking user ${userId} as offline: ${(error as Error).message}`,
      )
    }
  }
  ...
  const markUserInactive = async (): Promise<void> => {
    try {
      ...
      logger.error(
        `Error marking user ${userId} as inactive: ${(error as Error).message}`,
      )
    }
  }
  ...
  socket.on("activityPing", async (): Promise<void> => {
    try {
      ...
      logger.error(
        `Error processing activity ping for user ${userId}: ${(error as Error).message}`,
      )
    }
  })
  ...
  markUserOnline().catch((error) => {
    logger.error(
      `Error marking user ${userId} as online on connection: ${(error as Error).message}`,
    )
  })
  ...
  socket.on("disconnect", () => {
    markUserOffline().catch((error) => {
      logger.error(
        `Error marking user ${userId} as offline on disconnect: ${(error as Error).message}`,
      )
    })
