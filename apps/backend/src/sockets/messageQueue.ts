...
const isMessageQueueDisabled =
  process.env.DISABLE_MESSAGE_QUEUE === "true" ||
...
async function initializeQueue(): Promise<QueueConnection | null> {
  ...
  try {
    ...
    logger.error(
      `Failed to initialize message queue: ${(error as Error).message}`,
    )
...
async function publishMessage(message: Record<string, unknown>): Promise<void> {
  try {
    ...
    if (queue && queue.channel) {
      ...
      logger.info(
        `Message published to memory queue: ${JSON.stringify(message)}`,
      )
    ...
    logger.error(
      `Error publishing message to queue: ${(error as Error).message}`,
    )
    ...
    try {
      memoryMessageQueue.push(message)
      logger.info(
        `Message published to fallback memory queue: ${JSON.stringify(message)}`,
      )
    } catch (fallbackError) {
      logger.error(
        `Failed to publish to fallback queue: ${(fallbackError as Error).message}`,
      )
    }
  }
}
...
async function consumeMessages(
  ...
  try {
        ...
        while (memoryMessageQueue.length > 0) {
          ...
          if (message) {
            try {
              ...
              logger.info(
                `Message consumed from memory queue: ${JSON.stringify(message)}`,
              )
            } catch (handlerError) {
              logger.error(
    ...
    logger.error(
      `Error consuming messages from queue: ${(error as Error).message}`,
    )
