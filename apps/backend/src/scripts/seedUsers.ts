...
async function seedUsers(): Promise<void> {
  ...
  try {
    ...
    createdUsers.forEach((user) => {
      logger.info(
        `  ✅ ${user.name || user.username} (${user.email}) - ${user.subscriptionTier} (${user.subscription_status})`,
      )
    })
    ...
    users.forEach((user) => {
      ...
      logger.info(
        `     Subscription: ${user.subscriptionTier} (${user.subscription_status})`,
      )
      if (user.trial_end_date) {
        const daysLeft = Math.ceil(
          (user.trial_end_date.getTime() - new Date().getTime()) /
            (1000 * 60 * 60 * 24),
        )
      ...
      if (user.next_billing_date && user.subscription_status === "active") {
        logger.info(
          `     Next billing: ${user.next_billing_date.toLocaleDateString()}`,
        )
      }
      logger.info("")
    })
    ...
    logger.info(
      "  • Trial users with different days remaining (Test User, David, Henry)",
    )
