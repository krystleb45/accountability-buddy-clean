...
class IntegrationService {
  ...
  static async create(
    ...
    logger.info(
      `Integration created: ${newIntegration._id} for user ${userId}`,
      {
        type,
      },
    )
  ...
  static async listForUser(userId: string): Promise<IIntegration[]> {
    ...
    const integrations = await Integration.find({ user: userId }).sort({
      createdAt: -1,
    })
    return integrations
  }
   ...
   */
  static async test(integrationId: string, userId: string): Promise<void> {
