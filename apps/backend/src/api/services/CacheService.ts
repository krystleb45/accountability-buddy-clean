...
if (!isRedisDisabled) {
  try {
    ...
    logger.error(
      `Failed to load Redis client for CacheService: ${(error as Error).message}`,
    )
  }
} else {
...
const CacheService = {
    ...
    try {
      ...
      if (redisClient && !isRedisDisabled) {
        if (ttlSeconds > 0) {
          ...
          logger.info(
            `CacheService: set key=${key} (ttl=${ttlSeconds}s) [Redis]`,
          )
        } else {
        ...
        logger.info(
          `CacheService: set key=${key} (ttl=${ttlSeconds}s) [Memory]`,
        )
  ...
  async invalidateKeys(keys: string[]): Promise<void> {
    if (!Array.isArray(keys) || keys.length === 0) {
      throw new Error(
        "CacheService.invalidateKeys: non-empty array of keys required",
      )
    }
    ...
    try {
      if (redisClient && !isRedisDisabled) {
        ...
        logger.info(
          `CacheService: invalidated keys=[${keys.join(", ")}] [Redis]`,
        )
      } else {
        keys.forEach((key) => memoryCache.delete(key))
        logger.info(
          `CacheService: invalidated keys=[${keys.join(", ")}] [Memory]`,
        )
      }
    } catch (error) {
    ...
    try {
      if (redisClient && !isRedisDisabled) {
        ...
        if (cached && cached.expiry) {
          const ttl = Math.max(
            0,
            Math.floor((cached.expiry - Date.now()) / 1000),
          )
  ...
  async extendTTL(key: string, ttlSeconds: number): Promise<void> {
    if (!key || ttlSeconds <= 0) {
      throw new Error(
        "CacheService.extendTTL: key and positive ttlSeconds are required",
      )
    }
    ...
    try {
      if (redisClient && !isRedisDisabled) {
        ...
        logger.info(
          `CacheService: extended ttl key=${key} to ${ttlSeconds}s [Redis]`,
        )
      } else {
        const cached = memoryCache.get(key)
        if (cached) {
          ...
          logger.info(
            `CacheService: extended ttl key=${key} to ${ttlSeconds}s [Memory]`,
          )
        }
      }
    } catch (error) {
