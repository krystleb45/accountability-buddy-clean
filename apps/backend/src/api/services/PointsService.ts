...
// …
export async function addPoints (userId: string, points: number): Promise<IUser> {
  if (points <= 0) 
throw createError("Points must be a positive number", 400);
...
export async function subtractPoints (userId: string, points: number): Promise<IUser> {
  if (points <= 0) {
...
export async function redeemPoints (userId: string, rewardId: string): Promise<{ message: string; reward: any; userPoints: number }> {
  const user = await User.findById(userId);
  if (!user) {
    throw new Error("User not found.");
  }
