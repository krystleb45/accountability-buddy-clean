...
class AnonymousMoodService {
  ...
  static async submitMoodCheckIn(
    ...
    try {
      ...
      const hasSubmittedToday =
        await AnonymousMoodCheckIn.hasSubmittedToday(sessionId)
  ...
  static async getCommunityMoodData(): Promise<CommunityMoodResult> {
    try {
      ...
      return {
        ...
        encouragementMessage: this.getCommunityEncouragementMessage(
          todayData.averageMood,
          todayData.totalCheckIns,
        ),
      }
    } catch (error) {
   ...
   */
  static getCommunityEncouragementMessage(
    averageMood: number,
    totalCheckIns: number,
  ): string {
    if (totalCheckIns === 0) {
      ...
      const moodDistribution = [
        {
          mood: 1,
          count: stats.mood1,
          percentage: Math.round((stats.mood1 / totalSubmissions) * 100),
        },
        {
          mood: 2,
          count: stats.mood2,
          percentage: Math.round((stats.mood2 / totalSubmissions) * 100),
        },
        {
          mood: 3,
          count: stats.mood3,
          percentage: Math.round((stats.mood3 / totalSubmissions) * 100),
        },
        {
          mood: 4,
          count: stats.mood4,
          percentage: Math.round((stats.mood4 / totalSubmissions) * 100),
        },
        {
          mood: 5,
          count: stats.mood5,
          percentage: Math.round((stats.mood5 / totalSubmissions) * 100),
        },
      ]
