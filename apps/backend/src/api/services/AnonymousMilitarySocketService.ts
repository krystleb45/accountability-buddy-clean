...
export class AnonymousMilitarySocketService {
  ...
  private async handleSocketJoin(
    socket: any,
    data: { room: string; sessionId: string; displayName: string },
  ): Promise<void> {
    try {
  ...
  public handleCrisisDetection(room: string, _sessionId: string): void {
    this.io
      .of("/anonymous-military-chat")
      .to(room)
      .emit("crisis-resources", {
        message:
          "If you or someone you know is in crisis, help is available. Call 988 for the Veterans Crisis Line.",
        resources: [
          {
