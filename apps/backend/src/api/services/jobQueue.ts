    ...
    if (isRedisDisabled) {
      ...
      try {
        ...
        if (process.env.REDIS_URL) {
          ...
          throw new Error(
            "Redis configuration not found. REDIS_URL or REDIS_HOST must be set in Railway environment variables.",
          )
        }
        ...
        logger.info("🔴 Redis config:", {
          ...
          hasPassword: !!(typeof redisConfig === "object"
            ? redisConfig.password
            : redisConfig?.includes("@")),
        ...
        logger.error(
          `Failed to initialize Bull queue: ${(error as Error).message}`,
        )
