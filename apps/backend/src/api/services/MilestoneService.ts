...
class MilestoneService {
  ...
  static async listByUser(userId: string): Promise<IMilestone[]> {
    ...
    const milestones = await Milestone.find({ user: userId }).sort({
      createdAt: -1,
    })
    void LoggingService.logInfo(
      `Fetched ${milestones.length} milestones for user ${userId}`,
    )
    return milestones
  }
  ...
  static async add(
    ...
    void LoggingService.logInfo(
      `Milestone ${milestone._id} created for user ${userId}`,
      { title },
    )
    return milestone
  }
  ...
  static async update(
    ...
    await milestone.save()
    void LoggingService.logInfo(
      `Milestone ${milestoneId} updated for user ${userId}`,
      { updates },
    )
    return milestone
  }
  ...
  static async remove(userId: string, milestoneId: string): Promise<void> {
    ...
    const result = await Milestone.findOneAndDelete({
      _id: milestoneId,
      user: userId,
    })
    if (!result) {
    ...
    void LoggingService.logInfo(
      `Milestone ${milestoneId} deleted for user ${userId}`,
    )
  }
}
