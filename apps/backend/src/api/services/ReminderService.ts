...
export async function checkReminders(): Promise<void> {
  ...
  for (const rem of due) {
    ...
    try {
      // In-app
      await NotificationService.sendInAppNotification("system", userId, message)
      // Email
      ...
      void LoggingService.logInfo(`Sent & removed reminder ${rem.id}`, {
        userId,
      })
    } catch (err) {
      void LoggingService.logError("Error sending reminder", err as Error, {
        reminderId: rem.id,
      })
    }
  }
...
export async function scheduleReminderTask(
  ...
  if (Number.isNaN(dt.getTime())) {
    void LoggingService.logError(
      "Invalid remindAt date",
      new Error("Invalid remindAt date"),
    )
    return null
  }
  ...
  try {
    const task = cron.schedule(
      ...
      async () => {
        try {
          ...
          void LoggingService.logError("Error in cron reminder", err as Error, {
            reminderId: rem.id,
          })
        }
      },
    ...
    void LoggingService.logInfo(
      `Scheduled reminder ${rem.id} at ${rem.remindAt}`,
    )
    return task
  } catch (err) {
    void LoggingService.logError("Failed to schedule reminder", err as Error, {
      reminderId: rem.id,
    })
    return null
  }
