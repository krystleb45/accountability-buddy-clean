...
async function sendEmail(
  ...
  try {
    ...
    void LoggingService.logInfo(`Encouragement email sent to ${to}`, {
      subject,
    })
  } catch (err) {
...
async function sendInApp(receiverId: string, message: string): Promise<void> {
  try {
    ...
    void LoggingService.logInfo(
      `In-app encouragement sent to user ${receiverId}`,
      {
        message,
      },
    )
...
const EncouragementService = {
  ...
  async sendPeriodicEncouragement(): Promise<void> {
    try {
      ...
      void LoggingService.logInfo(
        "sendPeriodicEncouragement: all active users alerted",
      )
    } catch (err) {
      void LoggingService.logError(
