...
export const addMilestone = catchAsync(
  async (
    ...
    sendResponse(res, 201, true, "Milestone created", {
...
export const updateMilestone = catchAsync(
  async (
    ...
    const updates: Partial<
      Pick<IMilestone, "title" | "description" | "dueDate">
    > = {}
    ...
    sendResponse(res, 200, true, "Milestone updated successfully", {
      milestone: updated,
    })
  },
)
...
export const deleteMilestone = catchAsync(
  async (req: AuthenticatedRequest<{ milestoneId: string }>, res: Response) => {
    const userId = req.user!.id
    const { milestoneId } = req.params
