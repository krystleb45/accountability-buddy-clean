...
export const getAllTrackers = catchAsync(
  async (
    ...
    const trackers = await Tracker.find({ user: userId }).sort({
      createdAt: -1,
    })
...
export const createTracker = catchAsync(
  async (
    req: AuthenticatedRequest<unknown, unknown, { name: string }>,
    res: Response,
    _next: NextFunction,
  ): Promise<void> => {
    const userId = req.user?.id
...
export const addTrackingData = catchAsync(
  async (
    req: AuthenticatedRequest<unknown, unknown, Record<string, any>>,
    res: Response,
    _next: NextFunction,
  ): Promise<void> => {
