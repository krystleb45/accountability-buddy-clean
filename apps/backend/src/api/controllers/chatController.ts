...
export const getDashboardStats = catchAsync(
  async (req: Request, res: Response) => {
    ...
    logger.info(
      `✅ Dashboard stats fetched for user ${userId} (${user.subscriptionTier})`,
    )
...
export const getDashboardOverview = catchAsync(
  async (req: Request, res: Response) => {
    ...
    const recentCompletions = recentGoals.filter(
      (g) => g.completedAt && g.completedAt > thirtyDaysAgo,
    ).length
      ...
      activity: {
        ...
        completionRate:
          goalSummary.totalGoals > 0
            ? Math.round(
                (goalSummary.completedGoals / goalSummary.totalGoals) * 100,
              )
            : 0,
      },
    }
...
export const sendPrivateMessage = catchAsync(
  async (req: Request, res: Response) => {
    ...
    logger.info(
      `User ${userId} sending private message to ${friendId}: ${message}`,
    )
...
export const getPrivateChats = catchAsync(
  async (req: Request, res: Response) => {
    ...
    sendResponse(
      ...
      200,
      true,
      "Private chat history retrieved successfully",
      {
        friendId,
