 ...
 * @access  Private
 */
export const logDailyCheckIn = catchAsync(
  async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    const userId = req.user!.id!
    if (!mongoose.isValidObjectId(userId)) {
      sendResponse(res, 400, false, "Invalid User ID format.")
 ...
 */
export const resetUserStreak = catchAsync(
 ...
 * @access  Public
 */
export const getStreakLeaderboard = catchAsync(
  async (req: Request, res: Response): Promise<void> => {
    ...
    const { streaks, pagination } = await StreakService.getStreakLeaderboard(
      limit,
      page,
    )
