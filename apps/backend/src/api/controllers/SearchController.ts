...
export const globalSearch = catchAsync(
  async (
    ...
    switch (type) {
      case "user":
        ;({ items, pagination } = await SearchService.searchUsers(
          query,
          page,
          limit,
        ))
      ...
      case "group":
        ;({ items, pagination } = await SearchService.searchGroups(
          query,
          page,
          limit,
        ))
        sendResponse(res, 200, true, "Groups fetched", {
          groups: items,
          pagination,
        })
        return
      ...
      case "goal":
        ;({ items, pagination } = await SearchService.searchGoals(
          query,
          page,
          limit,
        ))
        sendResponse(res, 200, true, "Goals fetched", {
          goals: items,
          pagination,
        })
        return
      ...
      case "post":
        ;({ items, pagination } = await SearchService.searchPosts(
          query,
          page,
          limit,
        ))
...
export const searchUsers = catchAsync(
  async (req: Request<unknown, unknown, unknown, Q>, res: Response) => {
    const { query, page, limit } = req.query
    const { items: users, pagination } = await SearchService.searchUsers(
      query,
      page,
      limit,
    )
...
export const searchGroups = catchAsync(
  async (req: Request<unknown, unknown, unknown, Q>, res: Response) => {
    const { query, page, limit } = req.query
    const { items: groups, pagination } = await SearchService.searchGroups(
      query,
      page,
      limit,
    )
...
export const searchGoals = catchAsync(
  async (req: Request<unknown, unknown, unknown, Q>, res: Response) => {
    const { query, page, limit } = req.query
    const { items: goals, pagination } = await SearchService.searchGoals(
      query,
      page,
      limit,
    )
...
export const searchPosts = catchAsync(
  async (req: Request<unknown, unknown, unknown, Q>, res: Response) => {
    const { query, page, limit } = req.query
    const { items: posts, pagination } = await SearchService.searchPosts(
      query,
      page,
      limit,
    )
