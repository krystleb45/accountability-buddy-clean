...
export const sendMessage = catchAsync(
  async (req: AuthenticatedRequest, res: Response) => {
    const { receiverId, message, recipientId, groupId, content, messageType } =
      req.body
...
export const getMessagesWithUser = catchAsync(
  async (req: AuthenticatedRequest<{ userId: string }>, res: Response) => {
...
export const deleteMessage = catchAsync(
  async (req: AuthenticatedRequest<{ messageId: string }>, res: Response) => {
...
export const markMessagesAsRead = catchAsync(
  async (req: AuthenticatedRequest<{ userId: string }>, res: Response) => {
    ...
    const count = await MessageService.markMessagesAsRead(otherUserId, userId)
 ...
 */
export const getMessageThreads = catchAsync(
  async (req: AuthenticatedRequest, res: Response) => {
    const userId = req.user!.id
...
export const getMessagesInThread = catchAsync(
  async (req: AuthenticatedRequest<{ threadId: string }>, res: Response) => {
    const { threadId } = req.params
    const userId = req.user!.id
    const { limit = 50, page = 1, before } = req.query

    const result = await MessageService.getMessagesInThread(threadId, userId, {
      limit: Number.parseInt(limit as string, 10),
      page: Number.parseInt(page as string, 10),
 ...
 */
export const getMessageStats = catchAsync(
  async (req: AuthenticatedRequest, res: Response) => {
    const userId = req.user!.id
...
export const removeReaction = catchAsync(
  async (
    req: AuthenticatedRequest<{ messageId: string; emoji: string }>,
    res: Response,
  ) => {
 ...
 */
export const markMultipleMessagesAsRead = catchAsync(
  async (req: AuthenticatedRequest, res: Response) => {
    const { messageIds } = req.body
    const userId = req.user!.id

    const count = await MessageService.markMultipleMessagesAsRead(
      messageIds,
      userId,
    )
