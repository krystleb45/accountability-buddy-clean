...
export const sendMessage = catchAsync(async (req, res, next) => {
  const { chatroomId, message } = req.body as {
    chatroomId: string
    message: string
  }
  ...
  const newMsg = await MilitarySupportService.sendMessage(
    chatroomId,
    userId,
    message.trim(),
  )
...
export const createChatroom = catchAsync(async (req, res, next) => {
  ...
  const newRoom = await MilitarySupportService.createChatroom(
    name,
    description,
    userId,
  )
  sendResponse(res, 201, true, "Military chatroom created", {
    chatroom: newRoom,
  })
})
