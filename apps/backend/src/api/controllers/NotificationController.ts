...
export const sendNotification = catchAsync(
  async (
    req: AuthenticatedRequest<unknown, unknown, SendPayload>,
    res: Response,
    _next: NextFunction,
  ) => {
    ...
    if (!receiverId || !message.trim()) {
      sendResponse(
        ...
        400,
        false,
        "receiverId and non‐empty message are required",
      )
      return
    }
...
export const markNotificationsAsRead = catchAsync(
  async (
    req: AuthenticatedRequest<unknown, unknown, ReadPayload>,
    res: Response,
  ) => {
    const userId = req.user!.id
    ...
    const updatedCount = await NotificationService.markRead(
      userId,
      notificationIds,
    )
