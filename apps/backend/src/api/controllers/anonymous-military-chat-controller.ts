...
export const leaveRoom = catchAsync(
  async (req: AnonymousRequest, res: Response) => {
    ...
    const memberCount =
      await AnonymousMilitaryChatService.getRoomMemberCount(roomId)

    // Emit to WebSocket that user left
    const app = req.app
    const socketService = app.get("anonymousMilitarySocketService")
    if (socketService) {
      socketService.handleUserLeave(
        roomId,
        anonymousUser.displayName,
        memberCount,
      )
...
export const getRoomMemberCount = catchAsync(
