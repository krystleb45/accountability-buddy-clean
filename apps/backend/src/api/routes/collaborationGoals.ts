...
const validateGoalCreation = [
  ...
  check("participants", "Participants must be an array of user IDs").isArray({
    min: 1,
  }),
  check(
    "participants.*",
    "Each participant must be a valid Mongo ID",
  ).isMongoId(),
  check("target", "Target must be a positive integer").isInt({ min: 1 }),
]

const validateProgressUpdate = [
  param("id", "Invalid goal ID").isMongoId(),
  check("progress", "Progress must be a non-negative integer").isInt({
    min: 0,
  }),
]
...
router.post(
  ...
  catchAsync(
    async (
      ...
      const participantIds = (participants as string[]).map(
        (p) => new mongoose.Types.ObjectId(String(p)),
      )
...
router.put(
  ...
  catchAsync(
    async (
      ...
      const isParticipant = goal.participants.some((p) =>
  ...
  catchAsync(
    async (
      ...
      const goals = await CollaborationGoal.find({
        $or: [{ participants: userObjectId }, { createdBy: userObjectId }],
      }).sort({ createdAt: -1 })

      if (!goals.length) {
        res
          .status(404)
          .json({ success: false, message: "No collaboration goals found" })
        return
      }
