...
// ── Validation chains ────────────────────────────────────────────────────────
const validateEventIdParam = [param("eventId", "Invalid event ID").isMongoId()]
...
router.post(
  ...
  catchAsync(
    async (req: Request, res: Response, _next: NextFunction): Promise<void> => {
      ...
      const participantOids = (participants as string[]).map(
  ...
  catchAsync(
    async (req: Request, res: Response, _next: NextFunction): Promise<void> => {
      ...
      const events = await Event.find({
        $or: [{ "participants.user": userOid }, { createdBy: userOid }],
      }).sort({ createdAt: -1 })
