    ...
    try {
      ...
      void (async () => {
        try {
          ...
          logger.error(
            `Failed to connect session Redis: ${(error as Error).message}`,
          )
          console.error(
            "Session Redis connection failed - continuing with memory store",
          )
      ...
      logger.error(
        `Failed to setup Redis sessions: ${(error as Error).message}`,
      )
      ...
      logger.info(
        "✅ Session middleware initialized with fallback memory store",
      )
    }
  }
})()
...
function enhancedSessionMiddleware(
  req: Request,
  res: Response,
  next: NextFunction,
): void {
