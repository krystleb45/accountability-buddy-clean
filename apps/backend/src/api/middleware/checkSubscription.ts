...
function checkSubscription(requiredStatus: "trial" | "paid"): RequestHandler {
  return async (
    ...
    try {
      ...
      if (!dbUser) {
        logger.warn(
          `❌ checkSubscription: User not found in DB. UserID: ${req.user.id}`,
        )
      ...
      if (
        ...
        logger.warn(
          `❌ checkSubscription: Access denied. UserID: ${req.user.id}, Status: ${dbUser.subscription_status}`,
        )
        res.status(403).json({
          success: false,
      ...
      if (
        requiredStatus === "trial" &&
        !["active", "trial"].includes(dbUser.subscription_status)
      ) {
        logger.warn(
          `❌ checkSubscription: Trial access denied. UserID: ${req.user.id}, Status: ${dbUser.subscription_status}`,
        )
        res.status(403).json({
          success: false,
          message: "Access denied: Trial or paid subscription required.",
        })
        return // ✅ Ensures function returns void
      }

      logger.info(
        `✅ checkSubscription: User ${req.user.id} has valid access (${dbUser.subscription_status}). Proceeding...`,
      )
      next() // ✅ Proceed to next middleware
    } catch (error) {
      logger.error(
        `❌ checkSubscription: Unexpected error - ${(error as Error).message}`,
      )
