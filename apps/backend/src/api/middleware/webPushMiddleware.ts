...
if (!VAPID_PUBLIC_KEY || !VAPID_PRIVATE_KEY || !EMAIL) {
  throw new Error(
    "Missing VAPID keys or contact email in environment variables.",
  )
}
...
export async function sendNotification(
  ...
  try {
    ...
    if (!subscription || !payload) {
      res
        .status(400)
        .json({ message: "Subscription and payload are required." })
      return
    }
    ...
    res
      .status(500)
      .json({ message: "Failed to send notification.", error: errorMessage })
  }
}
