...
export async function updateLeaderboardInBulk(
  ...
  try {
    ...
    await Leaderboard.updateMany({ user: userId }, leaderboardUpdate, {
      upsert: true,
    ...
    logger.error(
      `Error updating leaderboard in bulk for user ${userId}: ${(error as Error).message}`,
    )
  }
}

// Utility function to update streak progress in bulk
export async function updateStreakProgressInBulk(
  userId: mongoose.Types.ObjectId,
  ...
  try {
    ...
    logger.error(
      `Error updating streak progress for user ${userId}: ${(error as Error).message}`,
    )
  }
}

// Utility function to award badges in bulk
export async function awardBadgesInBulk(
