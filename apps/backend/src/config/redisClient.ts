...
;(async () => {
  if (
    ...
    logger.info(
      "🚫 Redis disabled by environment flags - using comprehensive mock client",
    )
    ...
    try {
      ...
      rClient = createClient(redisConfig)

      // Attach event listeners for Redis
      rClient.on("connect", (): void => {
        logger.info("Connecting to Redis...")
      })

      rClient.on("ready", (): void => {
        logger.info("Redis client ready for use.")
      })

      rClient.on("error", (err: Error): void => {
      ...
      void (async (): Promise<void> => {
        try {
          ...
          logger.error(
            `Could not establish a connection to Redis: ${(error as Error).message}`,
          )
      ...
      const gracefulShutdown = async (): Promise<void> => {
        try {
          if (rClient && typeof rClient.quit === "function") {
            await rClient.quit()
