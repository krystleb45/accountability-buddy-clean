...
interface PointsResponse { points: number }
interface ErrorResponse  { message: string }
interface BackendResponse { points?: number; message?: string }
...
async function requireSession() {
  ...
  return session as NextAuthSession & { user: { id: string; accessToken: string } };
}
...
export async function POST(
  ...
  if (typeof pointsDelta !== 'number' || Number.isNaN(pointsDelta)) {
    return NextResponse.json({ message: 'Invalid pointsDelta' }, { status: 400 });
  }
