...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      "🔍 [PROXY] 4. Token option 1 (session.accessToken):",
      !!tokenOption1,
    )
    console.log(
      "🔍 [PROXY] 5. Token option 2 (session.user.accessToken):",
      !!tokenOption2,
    )
    console.log(
      "🔍 [PROXY] 6. Token option 3 (session.user?.accessToken):",
      !!tokenOption3,
    )
    console.log(
      "🔍 [PROXY] 7. Token option 4 (session.access_token):",
      !!tokenOption4,
    )

    const accessToken =
      tokenOption2 || tokenOption1 || tokenOption3 || tokenOption4
    console.log("🔍 [PROXY] 8. Final token selected:", !!accessToken)
    console.log(
      "🔍 [PROXY] 9. Token preview:",
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `📥 [PROXY] Express POST responded with status: ${response.status}`,
    )
    ...
    console.error(
      "💥 [PROXY ERROR] Error proxying create group request:",
      error,
    )
    return NextResponse.json(
