...
export async function POST(_request: NextRequest, { params }: Context) {
  try {
    ...
    if (!accessToken) {
      ...
      console.log('🔍 [LEAVE PROXY] Available session keys:', Object.keys(session));
      console.log('🔍 [LEAVE PROXY] Available user keys:', Object.keys(session.user || {}));
    ...
    console.log(`📥 [LEAVE PROXY] Express responded with status: ${response.status}`);
    ...
    console.error('💥 [LEAVE PROXY ERROR] Error proxying leave group request:', error);
