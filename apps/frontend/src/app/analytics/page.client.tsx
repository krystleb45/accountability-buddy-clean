    ...
    const fetchAnalytics = async (): Promise<void> => {
      try {
        ...
        if (trafficResp.success && trafficResp.data) {
          setTrafficData({ totalVisitors: trafficResp.data.totalGoalsCompleted });
        } else {
          throw new Error(trafficResp.message || 'Failed to load traffic data');
        }

        const activityResp: ApiResponse<AnalyticsData> = await getMilestoneAnalytics();
        if (activityResp.success && activityResp.data) {
          setUserActivity({ activeUsers: activityResp.data.totalMilestonesAchieved });
        } else {
          throw new Error(activityResp.message || 'Failed to load user activity data');
        }

        const revenueResp: ApiResponse<AnalyticsData> = await getCustomAnalytics(
          '2024-01-01',
          '2024-12-31',
          'revenue'
        );
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 p-6">
      ...
      <main className="mx-auto grid max-w-6xl grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        <section className="rounded-lg bg-white p-6 shadow transition hover:shadow-lg">
          ...
          <p className="mb-4 text-gray-600">Calculated custom metric “revenue.”</p>
          <p className="text-3xl font-bold text-gray-800">
