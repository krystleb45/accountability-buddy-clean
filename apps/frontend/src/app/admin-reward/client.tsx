...
export default function ClientAdminRewards(): React.ReactElement {
 ...
 /** add one */
  const handleAdd = async (): Promise<void> => {
    ...
    try {
      const created = await createAdminReward({ name, description, imageUrl, points });
      if (created) {
  ...
  return (
    <div className="min-h-screen bg-black p-6 text-white">
      <div className="mx-auto max-w-5xl">
        <h1 className="mb-6 text-3xl font-bold text-green-400">🎁 Admin Reward Management</h1>
              ...
              >
                {reward.imageUrl && (
                  <Image
                    ...
                    width={100}
                    height={100}
                    className="mx-auto mb-2 h-24 w-24 rounded object-contain"
                  />
                )}
                <h3 className="text-center text-lg font-semibold text-green-400">{reward.name}</h3>
                <p className="text-center text-sm text-gray-300">{reward.description}</p>
