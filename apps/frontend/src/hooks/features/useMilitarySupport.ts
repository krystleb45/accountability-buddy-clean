...
export interface UseMilitarySupportReturnType {
  ...
  updateResource: (id: string, updatedResource: Partial<MilitaryResource>) => Promise<void>;
  deleteResource: (id: string) => Promise<void>;
}

const useMilitarySupport = (): UseMilitarySupportReturnType => {
  ...
  const fetchResources = useCallback(async () => {
    ...
    setError(null);
    try {
      const { data } = await http.get<MilitaryResource[]>(API.MILITARY_SUPPORT.LIST);
      setResources(data);
    } catch (err: unknown) {
  ...
  const addResource = useCallback(async (newResource: Omit<MilitaryResource, 'id'>) => {
    setLoading(true);
    setError(null);
    try {
      const { data } = await http.post<MilitaryResource>(API.MILITARY_SUPPORT.CREATE, newResource);
