...
export async function fetchUserBadges(userId: string): Promise<Badge[]> {
  try {
    const resp = await http.get<Badge[]>(
...
export async function fetchUserProgress(
  ...
  try {
    const resp = await http.get<UserProgress>(
...
export async function awardBadge(
  ...
  try {
    await http.post(
      `/users/${encodeURIComponent(userId)}/badges/${encodeURIComponent(badgeId)}`,
      {},
    )
    return true
  } catch (error) {
