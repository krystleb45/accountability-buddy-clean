...
export async function checkIn(date?: string): Promise<Streak | null> {
  try {
    const resp = await http.post<Streak>('/streaks/check-in', date ? { date } : {});
    return resp.data;
  } catch (err) {
...
export async function fetchLeaderboard(page = 1, limit = 10): Promise<LeaderboardPage> {
  try {
    ...
    return { streaks: [], pagination: { totalEntries: 0, currentPage: 1, totalPages: 0 } };
  }
}
