...
export async function postXpEntry(xp: number, reason: string): Promise<XpEntry> {
  const resp: AxiosResponse<ApiResponse<XpEntry>> = await http.post('/xp-history', { xp, reason });
...
export async function getMyXpHistory(): Promise<XpEntry[]> {
  const resp: AxiosResponse<ApiResponse<XpEntry>> = await http.get('/xp-history');
...
export async function getAllXpHistory(): Promise<XpEntry[]> {
  const resp: AxiosResponse<ApiResponse<XpEntry>> = await http.get('/xp-history/all');
  if (!resp.data.success) {
