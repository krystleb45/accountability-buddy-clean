...
export async function createEvent(payload: {
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      console.error('❌ [eventApi::createEvent]', error.response?.data || error.message);
    } else {
...
export async function joinEvent(eventId: string): Promise<Event | null> {
  ...
  try {
    const resp = await http.post<Event>(`/events/${encodeURIComponent(eventId)}/join`);
    return resp.data;
  } catch (error) {
...
export async function leaveEvent(eventId: string): Promise<Event | null> {
  ...
  try {
    const resp = await http.post<Event>(`/events/${encodeURIComponent(eventId)}/leave`);
    return resp.data;
  } catch (error) {
