...
export type { UserProfile, UserBadge, Badge, UserScore, UserCheckIn };
...
export async function updateUserProfile(
  ...
  const resp: AxiosResponse<UserProfile> = await http.put('/users/profile', data);
  return resp.data;
}
...
export async function awardBadgeToUser(
  ...
  const resp: AxiosResponse<UserBadge> = await http.post('/users/badges/award', {
    userId,
    badgeId,
  });
  return resp.data;
...
export async function getLastCheckIn(): Promise<UserCheckIn> {
  const resp: AxiosResponse<UserCheckIn> = await http.get('/users/check-in/last');
  return resp.data;
}
...
export async function getUserPoints(): Promise<number> {
  const resp: AxiosResponse<{ points: number }> = await http.get('/users/points');
