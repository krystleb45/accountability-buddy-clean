...
export async function updateTracker(id: string, progress: number): Promise<Tracker> {
  if (!id.trim()) throw new Error('Tracker ID is required');
  try {
    const resp = await http.put<Tracker>(`/trackers/${encodeURIComponent(id)}`, { progress });
    return resp.data;
  } catch (error) {
    handleError(error);
  }
...
export async function deleteTracker(id: string): Promise<{ message: string }> {
  if (!id.trim()) throw new Error('Tracker ID is required');
  try {
    const resp = await http.delete<{ message: string }>(`/trackers/${encodeURIComponent(id)}`);
    return resp.data;
  } catch (error) {
    handleError(error);
  }
...
export async function addTrackingData(payload: Record<string, unknown>): Promise<TrackingData> {
  try {
...
export async function deleteTrackingData(id: string): Promise<{ message: string }> {
