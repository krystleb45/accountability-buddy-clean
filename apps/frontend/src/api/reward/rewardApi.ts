...
function logError(fn: string, error: unknown): void {
  if (axios.isAxiosError(error)) {
    console.error(`❌ [rewardApi::${fn}]`, error.response?.data || error.message);
  } else {
...
export async function redeemReward(rewardId: string): Promise<Reward | null> {
  ...
  try {
    const resp = await http.post<Reward>(`/rewards/${encodeURIComponent(rewardId)}/redeem`);
    return resp.data;
  } catch (err) {
...
/** POST /rewards */
export async function createReward(data: Partial<Reward>): Promise<Reward | null> {
  try {
