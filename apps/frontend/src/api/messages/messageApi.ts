 ...
 */
export async function fetchMessageThreads(filters?: {
  messageType?: "private" | "group"
  limit?: number
  page?: number
  search?: string
...
export async function fetchMessagesInThread(
  ...
  try {
    console.log(
      "🔍 fetchMessagesInThread: Starting request for thread:",
      threadId,
    )
...
export async function sendDirectMessage(
  ...
  if (!recipientId && !groupId) {
    console.error(
      "[messageApi::sendDirectMessage] either recipientId or groupId is required",
    )
...
export async function addMessageReaction(
  ...
  if (!messageId || !emoji) {
    console.error(
      "[messageApi::addMessageReaction] messageId and emoji are required",
    )
...
export async function fetchMessageStats(): Promise<MessageStats> {
  try {
    ...
    const res = await apiRequest<SendResponse<MessageStats>>(
      "/api/messages/stats",
    )
 ...
 */
export async function fetchMessages(communityId: string): Promise<Message[]> {
  if (!communityId) {
...
export async function sendMessage(
  ...
  if (!communityId || !content.trim() || !senderId) {
    console.error(
      "[messageApi::sendMessage] communityId, content, and senderId are required",
    )
