...
export async function fetchTaskById(taskId: string): Promise<Task> {
  ...
  try {
    const response = await http.get<Task>(`/tasks/${encodeURIComponent(taskId)}`);
    return response.data;
  } catch (error) {
    handleError(error);
  }
...
export async function updateTask(taskId: string, updates: Partial<TaskInput>): Promise<Task> {
  if (!taskId.trim()) throw new Error('Task ID is required.');
  try {
    const response = await http.put<Task>(`/tasks/${encodeURIComponent(taskId)}`, updates);
    return response.data;
  } catch (error) {
    handleError(error);
  }
...
export async function deleteTask(taskId: string): Promise<{ message: string }> {
  if (!taskId.trim()) throw new Error('Task ID is required.');
  try {
    const response = await http.delete<{ message: string }>(`/tasks/${encodeURIComponent(taskId)}`);
    return response.data;
  } catch (error) {
    handleError(error);
  }
