...
export async function getAnonymousRooms(): Promise<AnonymousChatRoom[]> {
  try {
    ...
    console.log(
      "🔍 [getAnonymousRooms] ANONYMOUS_API_BASE:",
      ANONYMOUS_API_BASE,
    )
    console.log(
      "🔍 [getAnonymousRooms] NEXT_PUBLIC_API_URL:",
      process.env.NEXT_PUBLIC_API_URL,
    )

    const resp =
      await axios.get<AnonymousApiResponse<{ rooms: AnonymousChatRoom[] }>>(url)
...
export async function joinAnonymousRoom(
  ...
  try {
    const resp = await axios.post<
      AnonymousApiResponse<{ memberCount: number }>
    >(
...
export async function getAnonymousRoomMemberCount(
  roomId: string,
): Promise<number> {
  try {
