...
export async function fetchFriends(userId: string): Promise<FollowUser[]> {
  ...
  try {
    ...
    if (resp.data) {
      ...
      } else if (resp.data.success && resp.data.data && resp.data.data.friends) {
 ...
 */
export async function fetchOnlineFriends(userId: string, limit: number = 5): Promise<FollowUser[]> {
  if (!userId) return [];

  try {
    console.log('🔍 API: Fetching online friends for userId:', userId, 'limit:', limit);
    ...
    try {
      ...
      console.error('❌ [friendApi::fetchOnlineFriends] Fallback also failed:', fallbackError);
      return [];
    }
  }
}
  ...
  try {
    ...
    if (resp.data) {
      if (Array.isArray(resp.data)) {
        ...
        requests = resp.data;
        console.log('📦 API: Using direct array response');
      } else if (resp.data.success && resp.data.data && resp.data.data.requests) {
        ...
        console.warn('⚠️ API: Unexpected friend requests response format:', resp.data);
        console.warn('⚠️ API: Available keys:', Object.keys(resp.data));
      }
    }
 ...
 */
export async function fetchFriendSuggestions(userId: string): Promise<FollowUser[]> {
  if (!userId) return [];
  try {
    ...
    if (resp.data) {
      if (Array.isArray(resp.data)) {
        ...
        console.log('📦 API: Using direct array response');
      } else if (resp.data.success && resp.data.data && resp.data.data.recommendedFriends) {
        ...
        console.log('📦 API: Using sendResponse format with direct data array');
      } else if (resp.data.recommendedFriends && Array.isArray(resp.data.recommendedFriends)) {
 ...
 */
export async function sendFriendRequest(userId: string, recipientId: string): Promise<boolean> {
 ...
 */
export async function declineFriendRequest(userId: string, requestId: string): Promise<boolean> {
  if (!userId || !requestId) return false;
  try {
