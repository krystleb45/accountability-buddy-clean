...
function handleError<T>(fn: string, error: unknown, fallback: ApiResponse<T>): ApiResponse<T> {
  if (axios.isAxiosError(error)) {
    console.error(`❌ [goalAnalyticsService::${fn}]`, error.response?.data || error.message);
  } else {
...
const GoalAnalyticsService = {
  ...
  async fetchGoalAnalyticsById(goalId: string): Promise<ApiResponse<GoalAnalytics>> {
    if (!goalId) {
  ...
  async fetchGoalAnalyticsByDateRange(
    ...
    try {
      const resp = await http.get<DateRangeAnalytics>('/analytics/goals/date-range', {
