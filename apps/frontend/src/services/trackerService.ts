...
const TrackerService = {
  ...
  async createTracker(name: string): Promise<Tracker> {
    ...
    const resp = await axiosRetry(() => http.post<Tracker>('/trackers', { name }));
    return resp.data;
  },
  ...
  async getTrackingData(): Promise<TrackingData[]> {
    const resp = await axiosRetry(() => http.get<TrackingData[]>('/trackers/data'));
    return resp.data;
  },

  /** POST /trackers/add */
  async addTrackingData(payload: Record<string, unknown>): Promise<TrackingData> {
    const resp = await axiosRetry(() => http.post<TrackingData>('/trackers/add', payload));
    return resp.data;
  },
  ...
  async deleteTrackingData(id: string): Promise<void> {
    ...
    await axiosRetry(() => http.delete(`/trackers/delete/${encodeURIComponent(id)}`));
  },
};
