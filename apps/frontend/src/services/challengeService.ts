...
const challengeService = {
  ...
  async createChallenge(payload: {
    ...
    try {
      const res = await http.post<{ challenge: Challenge }>(
        "/challenge",
        payload,
      )
  ...
  async getPublicChallenges(
    ...
    try {
      const res = await http.get<{ challenges: Challenge[] }>(
        "/challenge/public",
        {
  ...
  async joinChallenge(challengeId: string): Promise<ApiResponse<null>> {
    try {
      const res = await http.post<ApiResponse<null>>("/challenge/join", {
        challengeId,
      })
      return res.data
    } catch (err) {
  ...
  async leaveChallenge(challengeId: string): Promise<ApiResponse<null>> {
    try {
      const res = await http.post<ApiResponse<null>>("/challenge/leave", {
        challengeId,
      })
      return res.data
    } catch (err) {
