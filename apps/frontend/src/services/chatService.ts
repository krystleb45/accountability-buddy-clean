...
class ChatService {
  ...
  connect(chatId: string): void {
    if (!this.socket) {
      this.socket = io(process.env.NEXT_PUBLIC_API_BASE_URL || 'https://api.example.com');
      ...
      this.socket.on(
        ...
        (data: { chatId: string; messageId: string; userId: string }) => {
          if (data.chatId === chatId) {
            console.log(`Message ${data.messageId} was read by user ${data.userId}`);
          }
        },
      );
      ...
      this.socket.on(
        ...
        (data: {
          ...
          console.log(`Reactions updated for message ${data.messageId}:`, data.reactions);
        },
      );
    }
  }
  ...
  async fetchMessages(chatId: string): Promise<ChatMessage[]> {
    ...
    try {
      const response = await http.get<ChatMessage[]>(`/groups/${chatId}/messages`);
  ...
  async sendMessage(chatId: string, message: string): Promise<ChatMessage> {
    if (!chatId || !message.trim()) throw new Error('Chat ID and message content are required.');
    try {
      const response = await http.post<ChatMessage>(`/groups/${chatId}/messages`, {
        content: message,
        type: 'text',
      });
  ...
  async sendVoiceMessage(chatId: string, audioFile: File): Promise<ChatMessage> {
    if (!chatId || !audioFile) throw new Error('Chat ID and voice file are required.');
    try {
      ...
      if (!upload.data.fileUrl) throw new Error('Failed to upload voice message.');
      const msgResp = await http.post<ChatMessage>(`/groups/${chatId}/messages`, {
  ...
  async sendImageMessage(chatId: string, imageFile: File): Promise<ChatMessage> {
    if (!chatId || !imageFile) throw new Error('Chat ID and image file are required.');
    try {
      const formData = new FormData();
  ...
  async markMessageAsRead(chatId: string, messageId: string, userId: string): Promise<void> {
    try {
      await http.post(`/groups/${chatId}/messages/${messageId}/read`, { userId });
  ...
  async addReaction(
    ...
    try {
      const res = await http.post<{ reactions: { userId: string; emoji: string }[] }>(
        `/groups/${chatId}/messages/${messageId}/reactions`,
        { userId, emoji },
      );
