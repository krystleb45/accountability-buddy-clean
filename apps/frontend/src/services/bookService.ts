...
const bookService = {
  ...
  async getById(id: string): Promise<ApiResponse<{ book: Book }>> {
    ...
    try {
      ...
      return handleError("getById", err, {
        success: false,
  ...
  async update(
    id: string,
    data: Partial<
      Pick<Book, "title" | "author" | "category" | "description" | "coverImage">
    >,
  ): Promise<ApiResponse<{ book: Book }>> {
    if (!id) return { success: false, message: "Book ID is required" }
    try {
      const res = await http.put<ApiResponse<{ book: Book }>>(
        `/books/${id}`,
        data,
      )
      return res.data
  ...
  async like(id: string): Promise<ApiResponse<{ book: Book }>> {
    if (!id) return { success: false, message: "Book ID is required" }
    try {
      const res = await http.post<ApiResponse<{ book: Book }>>(
        `/books/${id}/like`,
      )
      return res.data
    } catch (err) {
  ...
  async addComment(
    ...
    if (!id || !text)
      return { success: false, message: "Book ID and text are required" }
    try {
      const res = await http.post<ApiResponse<{ book: Book }>>(
        `/books/${id}/comment`,
        { text },
      ...
      return handleError("addComment", err, {
        success: false,
