...
const handleError = <T>(fn: string, error: unknown, fallback: T): T => {
  if (axios.isAxiosError(error)) {
    console.error(`❌ [feedbackService::${fn}]`, error.response?.data || error.message);
  } else {
...
const FeedbackService = {
  ...
  async submitFeedback(feedback: string, userId: string): Promise<ApiResponse<null>> {
    if (!feedback.trim()) {
      return { success: false, message: 'Feedback cannot be empty.' };
    }
    try {
      const resp = await http.post<ApiResponse<null>>('/feedback/submit', { feedback, userId });
      return resp.data;
    } catch (err) {
  ...
  async deleteFeedback(feedbackId: string): Promise<ApiResponse<null>> {
    ...
    try {
      const resp = await http.delete<ApiResponse<null>>(`/feedback/${feedbackId}`);
      return resp.data;
    } catch (err) {
