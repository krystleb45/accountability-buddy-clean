...
// -----------------------------------------------------------------------------
const handleApiError = <T>(fn: string, error: unknown, fallback: T): T => {
  if (axios.isAxiosError(error)) {
    console.error(`❌ [blogService::${fn}]`, error.response?.data ?? error.message);
  } else {
...
const BlogService = {
  ...
  async addComment(id: string, text: string): Promise<ApiResponse<Comment>> {
    if (!id || !text) return { success: false, message: 'Post ID and text are required' };
    try {
   ...
   */
  async deleteComment(id: string, commentId: string): Promise<ApiResponse<null>> {
    if (!id || !commentId)
