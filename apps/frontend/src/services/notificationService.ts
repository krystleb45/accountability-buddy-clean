...
async function axiosRetry<T>(
  ...
  while (attempt < retries) {
    try {
      ...
      const shouldRetry =
        isAxiosErr && status !== undefined && status >= 500 && attempt < retries - 1;
      if (shouldRetry) {
        // exponential backoff
...
const NotificationService = {
  ...
  async getUserNotifications(): Promise<Notification[]> {
    const resp = await axiosRetry(() => http.get<Notification[]>('/notifications'));
    return resp.data;
  },
  ...
  async markNotificationsAsRead(notificationIds: string[]): Promise<number> {
    const resp = await axiosRetry(() =>
      http.patch<{ updatedCount: number }>('/notifications/read', { notificationIds }),
    );
  ...
  async deleteNotification(notificationId: string): Promise<void> {
    await axiosRetry(() => http.delete(`/notifications/${encodeURIComponent(notificationId)}`));
  },
