...
const NotificationTriggerService = {
  ...
  async sendDailyStreakReminder(userId: string): Promise<void> {
    try {
      await http.post<ApiResponse>('/notifications/daily-streak-reminder', { userId });
    } catch (err) {
      console.error('[NotificationTriggerService.sendDailyStreakReminder] ', err);
  ...
  async sendLevelUpNotification(userId: string, level: number): Promise<void> {
    try {
      ...
      console.error('[NotificationTriggerService.sendLevelUpNotification] ', err);
      throw new Error('Failed to send level-up notification.');
    }
  },

  /** POST /notifications/badge-unlock */
  async sendBadgeUnlockNotification(userId: string, badgeName: string): Promise<void> {
    try {
      await http.post<ApiResponse>('/notifications/badge-unlock', { userId, badgeName });
    } catch (err) {
      console.error('[NotificationTriggerService.sendBadgeUnlockNotification] ', err);
      throw new Error('Failed to send badge-unlock notification.');
    }
  },

  /** POST /notifications/email */
  async sendCustomEmail(email: string, subject: string, text: string): Promise<void> {
    try {
      await http.post<ApiResponse>('/notifications/email', { email, subject, text });
    } catch (err) {
