...
const PartnerService = {
  ...
  async notifyPartner(
    ...
    await retry(() =>
      http.post("/partners/notify", { partnerId, goal, milestone }),
    )
  },
  ...
  async acceptPartnerRequest(requestId: string): Promise<void> {
    ...
    await retry(() =>
      http.post(`/partners/accept/${encodeURIComponent(requestId)}`),
    )
  },
  ...
  async declinePartnerRequest(requestId: string): Promise<void> {
    if (!requestId) throw new Error("requestId is required")
    await retry(() =>
      http.post(`/partners/decline/${encodeURIComponent(requestId)}`),
    )
  },
}
