...
const handleError = <T>(fn: string, error: unknown, fallback: T): T => {
  if (axios.isAxiosError(error)) {
    console.error(`❌ [directChatService::${fn}]`, error.response?.data || error.message);
  } else {
...
const DirectChatService = {
  ...
  async sendToGroup(chatId: string, message: string): Promise<ApiResponse<DirectMessage>> {
  ...
  async sendToFriend(friendId: string, message: string): Promise<ApiResponse<DirectMessage>> {
    if (!friendId || !message.trim()) {
      return { success: false, message: 'friendId and message are required' };
    }
    try {
      const { data } = await http.post<DirectMessage>(`/chat/private/${friendId}`, { message });
      return { success: true, data };
    } catch (err) {
  ...
  async fetchPrivateHistory(friendId: string): Promise<ApiResponse<DirectMessage[]>> {
    if (!friendId) {
      return { success: false, message: 'friendId is required', data: [] };
    }
    try {
      const { data } = await http.get<DirectMessage[]>(`/chat/private/${friendId}`);
      return { success: true, data };
    } catch (err) {
