...
const badgeService = {
  ...
  async awardBadge(userId: string, badgeId: string): Promise<Badge | null> {
    ...
    try {
      const { data } = await http.post<UserBadge>("/badges/award", {
        userId,
        badgeId,
      })
      return {
  ...
  async updateFavoriteBadges(
    userId: string,
    badgeIds: string[],
  ): Promise<Badge[]> {
    try {
      const { data } = await http.patch<Badge[]>(
        `/users/${userId}/favorite-badges`,
        { badgeIds },
      )
      return data
    } catch (err) {
  ...
  async toggleFavoriteBadge(userId: string, badgeId: string): Promise<Badge[]> {
    try {
      const { data } = await http.post<Badge[]>(
        `/users/${userId}/favorite-badges/toggle`,
        {
          badgeId,
        },
      )
