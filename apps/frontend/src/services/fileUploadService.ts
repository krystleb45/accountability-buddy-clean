...
const handleError = <T>(fn: string, error: unknown, fallback: ApiResponse<T>): ApiResponse<T> => {
  if (axios.isAxiosError(error)) {
    console.error(`❌ [fileUploadService::${fn}]`, error.response?.data || error.message);
  } else {
...
const FileUploadService = {
  ...
  async uploadFile(file: File): Promise<ApiResponse<UploadResponse>> {
    ...
    try {
      const resp = await http.post<ApiResponse<UploadResponse>>('/files/upload', formData, {
  ...
  async deleteFile(fileName: string): Promise<ApiResponse<null>> {
    ...
    try {
      const resp = await http.delete<ApiResponse<null>>(`/files/${encodeURIComponent(fileName)}`);
      return resp.data;
    } catch (err) {
