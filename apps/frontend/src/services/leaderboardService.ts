...
const handleError = <T>(fn: string, error: unknown, fallback: ApiResponse<T>): ApiResponse<T> => {
  if (axios.isAxiosError(error)) {
    console.error(`❌ [leaderboardService::${fn}]`, error.response?.data || error.message);
    return {
      success: false,
...
const LeaderboardService = {
  ...
  async getLeaderboard(
    page = 1,
    limit = 10,
  ): Promise<ApiResponse<{ leaderboard: LeaderboardEntry[]; pagination: Pagination }>> {
    try {
  ...
  async resetLeaderboard(): Promise<ApiResponse<null>> {
    try {
      ...
      return handleError('resetLeaderboard', err, { success: false, data: null });
    }
  },
