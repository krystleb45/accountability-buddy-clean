...
const handleError = <T>(fn: string, error: unknown, fallback: T): T => {
  if (axios.isAxiosError(error)) {
    console.error(`❌ [matchService::${fn}]`, error.response?.data || error.message);
  } else {
...
const MatchService = {
  /** POST /matches */
  async create(user1: string, user2: string, status = 'pending'): Promise<Match> {
    try {
  ...
  async updateStatus(matchId: string, status: string): Promise<Match> {
    try {
      const resp = await http.patch<Match>(`/matches/${encodeURIComponent(matchId)}/status`, {
        status,
      });
      return resp.data;
    } catch (err) {
