...
const RewardsSection: React.FC = () => {
  ...
  return (
    <section className={styles.rewardsContainer} aria-labelledby="rewards-section-title">
      ...
      {rewards.length > 0 ? (
        <div className={styles.rewardsGrid}>
          {rewards.map((reward) => (
            <div key={reward.id} className={styles.rewardCard}>
              ...
              <p className={styles.rewardPoints}>🪙 {reward.points.toLocaleString()} pts</p>
              {reward.description && (
        ...
        <p className={styles.noRewardsText}>No rewards available at this time.</p>
      )}
    </section>
  );
};
