...
const ChallengeCard: React.FC<ChallengeCardProps> = ({
  ...
  return (
    <motion.div
      className={`
        relative flex flex-col rounded-lg bg-gray-800 p-4 shadow-md
    ...
    >
      {isJoined && (
        <span
          className={`
            absolute top-2 right-2 rounded-full bg-green-500 px-2 py-1 text-xs
      ...
      <div
        className={`
          mb-4 flex flex-col
          sm:mb-0 sm:flex-row sm:items-center sm:space-x-4
        `}
      >
        <div
          className={`
        ...
        >
          {isJoined ? (
            <button
              type="button"
              className={`
                w-full rounded-md bg-red-600 py-2 text-white
                sm:w-auto
              `}
              onClick={handleLeave}
            >
            ...
            <button
              type="button"
              className={`
                w-full rounded-md bg-green-600 py-2 text-white
                sm:w-auto
              `}
              onClick={handleJoin}
            >
