...
const Feed: React.FC<FeedProps> = ({ posts }) => {
  ...
  if (loading) return <p className="text-center text-gray-400">Loading activities...</p>;
  ...
  return (
    <div className="feed-container rounded-lg bg-gray-900 p-6 shadow-lg">
      <h2 className="mb-4 text-2xl font-bold text-kelly-green">Recent Activities</h2>
      {feedPosts.map((post) => (
        <Post
          key={post.id}
          id={post.id}
