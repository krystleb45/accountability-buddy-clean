...
const Reminders: React.FC = () => {
  ...
  useEffect(() => {
    const fetchReminders = async (): Promise<void> => {
      ...
      setError('');
      try {
        const response = await axios.get<Reminder[]>(`${process.env.REACT_APP_API_URL}/reminders`);
    ...
    <div className="reminders">
        ...
        <ul className="reminder-list">
          {reminders.map((reminder) => (
            <li key={reminder.id} className="reminder-item">
              ...
              <p className="reminder-time">{new Date(reminder.time).toLocaleString()}</p>
              <button onClick={() => handleDeleteReminder(reminder.id)} className="delete-button">
                Delete
              </button>
            </li>
          ))}
      ...
      <div className="new-reminder-form">
        ...
        <input
          ...
          className="input"
        />
        <button onClick={handleSaveReminder} disabled={saving} className="save-button">
