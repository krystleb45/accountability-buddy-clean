...
declare global {
  ...
  namespace Cypress {
    ...
    interface Chainable<Subject = any, T extends Element = Element> {
      loginViaSession: (email: string, password?: string) => Chainable<Element>
      loginAs: (email: string, password?: string) => Chainable<Element>
      simulateApiFailure: (endpoint: string) => Chainable<Element>
      uploadFileCustom: (
        selector: string,
        fileName: string,
      ) => Chainable<Element>
      sendMessageInChatroom: (message: string) => Chainable<Element>
      waitForElement: (
        selector: string,
        timeout?: number,
      ...
      clearLocalStorageCustom: () => Chainable<Element>
      navigateToPayments: () => Chainable<Cypress.AUTWindow>
      navigateToMilitarySupport: () => Chainable<Cypress.AUTWindow>
      simulateSuccessfulPayment: (
        amount: number,
        method: string,
      ) => Chainable<Element>
      simulateFailedPayment: (errorMessage: string) => Chainable<Element>
      verifyDisclaimer: (text: string) => Chainable<Element>
      clearChatHistory: () => Chainable<Element>
      verifyResourceLinks: (
        resources: { name: string; url: string }[],
      ) => Chainable<Element>
      mockAccessControl: (role: string) => Chainable<Element>
...
Cypress.Commands.add("uploadFileCustom", (selector, fileName) => {
  ;(cy.get(selector) as any).attachFile(fileName)
})
