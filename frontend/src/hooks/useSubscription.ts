...
export interface SubscriptionStatus {
  ...
  trialEnd?: string;
  subscription_status: 'active' | 'canceled' | 'past_due' | 'trialing' | 'incomplete' | 'trial' | 'expired'; // ← Changed from 'status'
...
interface UseSubscriptionReturn {
  ...
  createCheckoutSession: (planId: string, billingCycle?: 'monthly' | 'yearly') => Promise<{ sessionUrl: string } | null>;
  changePlan: (newPlanId: string, billingCycle?: 'monthly' | 'yearly') => Promise<void>;
...
export default function useSubscription(): UseSubscriptionReturn {
  ...
  const fetchPlans = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const plansData = response.plans || (response as any).data?.plans || response as any;
  ...
  const fetchStatus = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const statusData = response.status || (response as any).data?.status || response as any;
  ...
  const fetchLimits = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const limitsData = response.limits || (response as any).data?.limits || response as any;
  ...
  const createCheckoutSession = useCallback(
    async (planId: string, billingCycle: 'monthly' | 'yearly' = 'monthly'): Promise<{ sessionUrl: string } | null> => {
      setLoading(true);
      setError(null);
      try {
        ...
        const sessionData = response.sessionUrl ? response : (response as any).data;
        return sessionData;
      } catch (err) {
  ...
  const changePlan = useCallback(
    async (newPlanId: string, billingCycle: 'monthly' | 'yearly' = 'monthly'): Promise<void> => {
      setLoading(true);
      setError(null);
      try {
        ...
        const errorMessage = err.response?.data?.message || err.message || 'Failed to change subscription plan.';
