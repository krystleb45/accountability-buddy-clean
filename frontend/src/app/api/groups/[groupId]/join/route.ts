...
export async function POST(_request: NextRequest, { params }: Context) {
  try {
    ...
    if (!accessToken) {
      ...
      console.log('🔍 [JOIN PROXY] Available session keys:', Object.keys(session));
      console.log('🔍 [JOIN PROXY] Available user keys:', Object.keys(session.user || {}));
      return NextResponse.json({ error: 'No access token available' }, { status: 401 });
    }

    const { groupId } = params;
    const expressUrl = `${EXPRESS_API_URL}/api/groups/${groupId}/join`;

    console.log(`🚀 [JOIN PROXY] POST ${expressUrl}`);
    console.log('🔑 [JOIN PROXY] Using token:', accessToken.substring(0, 20) + '...');
    ...
    console.log(`📥 [JOIN PROXY] Express responded with status: ${response.status}`);
    ...
    console.error('💥 [JOIN PROXY ERROR] Error proxying join group request:', error);
