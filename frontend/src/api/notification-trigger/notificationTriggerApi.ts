...
function logError(fn: string, error: unknown): void {
  if (axios.isAxiosError(error)) {
    console.error(`❌ [notificationTriggerApi::${fn}]`, error.response?.data || error.message);
  } else {
...
export async function triggerDailyStreakReminder(userId: string): Promise<boolean> {
...
export async function triggerLevelUpNotification(userId: string, level: number): Promise<boolean> {
  if (!userId) return false;
  try {
