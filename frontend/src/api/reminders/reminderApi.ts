...
function logError(fn: string, error: unknown): void {
  if (axios.isAxiosError(error)) {
    console.error(`❌ [reminderApi::${fn}]`, error.response?.data || error.message);
  } else {
...
/** POST /reminders */
export async function createReminder(reminderData: Partial<Reminder>): Promise<Reminder | null> {
  try {
...
export async function disableReminder(reminderId: string): Promise<Reminder | null> {
  try {
    const resp = await http.put<Reminder>(`/reminders/${encodeURIComponent(reminderId)}/disable`);
    return resp.data;
  } catch (err) {
...
/** DELETE /reminders/:id */
export async function deleteReminder(reminderId: string): Promise<{ message: string } | null> {
  try {
