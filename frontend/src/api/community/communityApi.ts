 ...
 * GET /communities
 */
export const fetchPublicCommunities = async (): Promise<Community[]> => {
  try {
    const res = await http.get<SendResponse<{ communities: Community[] }>>(
      '/communities'
    );
...
export const fetchCommunityStats = async (): Promise<CommunityStats> => {
  try {
    ...
    const [friendsResponse, groupsResponse, messagesResponse] = await Promise.allSettled([
      http.get('/friends'),
    ...
    if (friendsResponse.status === 'fulfilled') {
      ...
      if (Array.isArray(friendsData)) {
        ...
        onlineFriends = friendsData.filter((friend: any) => friend.activeStatus === 'online').length;
      } else if (friendsData?.data?.friends) {
        totalFriends = friendsData.data.friends.length;
        onlineFriends = friendsData.data.friends.filter((friend: any) => friend.activeStatus === 'online').length;
      } else if (friendsData?.friends) {
        totalFriends = friendsData.friends.length;
        onlineFriends = friendsData.friends.filter((friend: any) => friend.activeStatus === 'online').length;
      }
    } else {
    ...
    return { totalFriends: 0, activeGroups: 0, unreadMessages: 0, onlineFriends: 0 };
  }
};
 ...
 */
export const fetchRecentMessages = async (limit: number = 5): Promise<RecentMessage[]> => {
  try {
    ...
    const recentMessages: RecentMessage[] = messages.map((message: any) => {
      ...
      if (message.senderAvatar || message.sender?.avatar) {
        recentMessage.senderAvatar = message.senderAvatar || message.sender?.avatar;
      }
    ...
    console.log('✅ [communityApi] Recent messages loaded:', recentMessages.length);
 ...
 */
export const fetchOnlineFriends = async (limit: number = 5): Promise<OnlineFriend[]> => {
  try {
    ...
    const transformedFriends: OnlineFriend[] = onlineFriends.map((friend: any) => {
    ...
    console.log('✅ [communityApi] Online friends loaded:', transformedFriends.length);
