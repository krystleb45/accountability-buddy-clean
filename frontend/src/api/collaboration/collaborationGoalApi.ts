...
export async function createCollaborationGoal(
  ...
  try {
    const response = await http.post<{ data: CollaborationGoal }>('/collaboration-goals', goalData);
...
export async function getUserCollaborationGoals(page = 1, limit = 10): Promise<GoalsResponse> {
  try {
...
export async function getCollaborationGoalById(goalId: string): Promise<CollaborationGoal> {
  try {
    const response = await http.get<{ data: CollaborationGoal }>(`/collaboration-goals/${encodeURIComponent(goalId)}`);
    return response.data.data;
  } catch (error: unknown) {
...
export async function deleteCollaborationGoal(goalId: string): Promise<{ message: string }> {
  try {
    const response = await http.delete<{ message: string }>(`/collaboration-goals/${encodeURIComponent(goalId)}`);
    return response.data;
  } catch (error: unknown) {
