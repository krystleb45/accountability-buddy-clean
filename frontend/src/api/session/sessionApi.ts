...
function logError(fn: string, error: unknown): void {
  if (axios.isAxiosError(error)) {
    console.error(`❌ [sessionApi::${fn}]`, error.response?.data || error.message);
  } else {
...
export async function login(
  ...
  try {
    ...
    return resp.data.success ? { token: resp.data.token, sessionId: resp.data.sessionId! } : null;
  } catch (err) {
    logError('login', err);
    return null;
  }
...
export async function deleteAllSessions(currentSessionId: string): Promise<boolean> {
  try {
...
export async function fetchSession(sessionId: string): Promise<Session | null> {
  try {
    const resp = await http.get<{ success: boolean; data: Session }>(`/session/${encodeURIComponent(sessionId)}`);
...
export async function deleteSession(sessionId: string): Promise<boolean> {
  try {
    const resp = await http.delete<{ success: boolean; message?: string }>(`/session/${encodeURIComponent(sessionId)}`);
    return resp.data.success;
  } catch (err) {
