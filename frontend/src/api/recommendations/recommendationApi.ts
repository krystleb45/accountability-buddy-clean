...
import type { Book, Goal, BlogPost, FriendSuggestion } from '@/services/recommendationService';

function logError(fn: string, error: unknown): void {
  if (axios.isAxiosError(error)) {
    console.error(`❌ [recommendationApi::${fn}]`, error.response?.data || error.message);
  } else {
...
export async function fetchFriendRecommendations(): Promise<FriendSuggestion[]> {
  try {
