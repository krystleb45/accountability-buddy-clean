...
export function safeSetItem<T>(key: string, value: T): boolean {
  ...
  try {
    ...
    console.error(`Failed to set localStorage item "${key}":`, (err as Error).message);
    return false;
  }
}
...
export function safeGetItem<T = unknown>(key: string): T | null {
  ...
  try {
    ...
    console.error(`Failed to get localStorage item "${key}":`, (err as Error).message);
    return null;
  }
}
...
export function safeRemoveItem(key: string): boolean {
  ...
  try {
    ...
    console.error(`Failed to remove localStorage item "${key}":`, (err as Error).message);
    return false;
  }
}
...
export function localStorageKeyExists(key: string): boolean {
  ...
  try {
    ...
    console.error(`Error checking existence of "${key}":`, (err as Error).message);
    return false;
  }
}
