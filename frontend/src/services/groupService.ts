...
const handleError = <T>(
  ...
  if (axios.isAxiosError(error)) {
    console.error(`❌ [groupService::${fn}]`, error.response?.data || error.message);
  } else {
...
const GroupService = {
  /** POST /group/create */
  async createGroup(name: string, interests: string[]): Promise<ApiResponse<Group>> {
    try {
  ...
  async fetchUnreadCounts(): Promise<Record<string, number>> {
    try {
      const resp = await http.get<Record<string, number>>('/group/unread-counts');
      return resp.data;
    } catch (err) {
