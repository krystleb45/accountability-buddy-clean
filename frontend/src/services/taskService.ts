...
const TaskService = {
  ...
  async createTask(data: TaskInput): Promise<Task> {
    ...
    const resp = await axiosRetry(() => http.post<Task>('/tasks', data, { headers }));
    return resp.data;
  },
  ...
  async updateTask(taskId: string, updates: Partial<TaskInput>): Promise<Task> {
    ...
    const headers = getAuthHeader();
    const resp = await axiosRetry(() =>
      http.put<Task>(`/tasks/${encodeURIComponent(taskId)}`, updates, { headers }),
    );
    return resp.data;
  },
  ...
  async deleteTask(taskId: string): Promise<void> {
    if (!taskId.trim()) throw new Error('Task ID is required');
    const headers = getAuthHeader();
    await axiosRetry(() => http.delete(`/tasks/${encodeURIComponent(taskId)}`, { headers }));
  },
};
