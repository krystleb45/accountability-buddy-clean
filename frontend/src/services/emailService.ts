...
const EmailService = {
  ...
  async sendEmail(data: SendEmailPayload): Promise<void> {
    try {
      ...
      const resp = await http.post<{ success: boolean; message?: string }>('/email/send', data);
      ...
      if (axios.isAxiosError(error)) {
        console.error('❌ [EmailService::sendEmail]', error.response?.data || error.message);
        throw new Error(
          (error.response?.data as { message?: string })?.message || 'Failed to send email',
        );
      }
