...
export async function contactSupport(supportData: SupportData): Promise<void> {
  ...
  if (!name || !email || !subject || !message) {
    throw new Error('All required fields (name, email, subject, message) must be provided.');
  }
  await retry(() =>
    http.post<ApiResponse<null>>('/support/contact', supportData, { headers: getAuthHeader() }),
  );
}
...
export async function getSupportTickets(): Promise<SupportTicket[]> {
  const resp = await retry(() =>
    http.get<ApiResponse<SupportTicket[]>>('/support/tickets', { headers: getAuthHeader() }),
  );
 ...
 */
export async function getTicketDetails(ticketId: string): Promise<TicketDetails> {
  if (!ticketId) {
    throw new Error('Ticket ID is required to fetch ticket details.');
  }
  const resp = await retry(() =>
    http.get<ApiResponse<TicketDetails>>(`/support/tickets/${encodeURIComponent(ticketId)}`, {
...
export async function updateSupportTicket(
  ...
  await retry(() =>
    http.put<ApiResponse<null>>(`/support/tickets/${encodeURIComponent(ticketId)}`, updateData, {
      headers: getAuthHeader(),
    }),
  );
}
