...
const UserService = {
  ...
  async fetchProfile(): Promise<UserProfile> {
    const { data } = await axiosRetry(() => http.get<UserProfile>('/users/profile'));
    return data;
  },
  ...
  async updateProfile(updates: Partial<UserProfile>): Promise<UserProfile> {
    ...
    const { data } = await axiosRetry(() => http.put<UserProfile>('/users/profile', updates));
    return data;
  },

  /** GET /users/all */
  async fetchAll(): Promise<UserProfile[]> {
    const { data } = await axiosRetry(() => http.get<UserProfile[]>('/users/all'));
    return data;
  },
  ...
  async fetchUserBadges(userId: string): Promise<UserBadge[]> {
    ...
    const { data } = await axiosRetry(() => http.get<UserBadge[]>(`/users/${userId}/badges`));
    return data;
  },
  ...
  async fetchLeaderboard(): Promise<UserScore[]> {
    const { data } = await axiosRetry(() => http.get<UserScore[]>('/users/leaderboard'));
    return data;
  },
  ...
  async getLastCheckIn(): Promise<UserCheckIn> {
    try {
      const { data } = await axiosRetry(() => http.get<UserCheckIn>('/users/check-in/last'));
      return data;
    } catch {
  ...
  async logCheckIn(): Promise<UserCheckIn> {
    const { data } = await axiosRetry(() => http.post<UserCheckIn>('/users/check-in'));
    return data;
  },
