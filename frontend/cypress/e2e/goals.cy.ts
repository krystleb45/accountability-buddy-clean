...
describe('Goal Tracking Features', () => {
  ...
  it('updates a goal successfully', () => {
    ...
    cy.get('@goalToEdit').find('[data-cy="edit-goal-input"]').clear().type(updatedGoalName);
    cy.get('@goalToEdit').find('[data-cy="save-goal-button"]').click();

    cy.get('[data-cy="goal-item"]').contains(updatedGoalName).should('be.visible');
  });
  ...
  it('shows progress for completed goals', () => {
    cy.get('[data-cy="progress-bar"]').invoke('width').should('be.greaterThan', 0);
  });
  ...
  it('displays a confirmation prompt before deleting a goal', () => {
    ...
    cy.contains(/are you sure you want to delete this goal/i).should('be.visible');
